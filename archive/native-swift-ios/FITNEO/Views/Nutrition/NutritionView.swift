...
struct NutritionView: View {
    ...
    // New: gated scanner views
    @State private var showBarcodeScanner = false
    @State private var showFoodPhotoScanner = false
    ...
    var body: some View {
        ...
        .confirmationDialog("Scan Meal", isPresented: $showScanner, titleVisibility: .visible) {
            Button("AI Photo Scan (Cooked Meals)") {
                openFoodPhotoScanner()
            }
            Button("Barcode Scanner (Packaged Foods)") {
                openBarcodeScanner()
            }
        ...
        .fullScreenCover(isPresented: $showBarcodeScanner) {
            BarcodeScannerView(onFoodLogged: { result in
                let entry = FoodEntry(
                    id: UUID(),
                    foodID: "barcode_\(UUID().uuidString.prefix(6))",
                    name: result.foodName,
                    mealType: scannerMeal ?? .lunch,
                    portion: 1,
        ...
        // Vision food scanner
        .fullScreenCover(isPresented: $showFoodPhotoScanner) {
            FoodPhotoScannerView(onFoodLogged: { components, totals in
                let meal = scannerMeal ?? .lunch
                for component in components {
                    let entry = FoodEntry(
                        id: UUID(),
                        foodID: "vision_\(UUID().uuidString.prefix(6))",
                        name: component.name,
                        mealType: meal,
                        portion: 1,
                        calories: Int(component.calories),
        ...
        .fullScreenCover(isPresented: $showDisclaimer) {
            NutritionDisclaimerPopup(isPresented: $showDisclaimer)
        }
        .alert("Scan Failed", isPresented: .init(
            get: { scanError != nil },
            set: { if !$0 { scanError = nil } }
        )) {
            Button("Retry") { scanError = nil }
    ...
    // MARK: - Scanner gate enforcement (Step 3 + 4 from prompt)

    private func openBarcodeScanner() {
        let hasAccepted = UserDefaults.standard.bool(forKey: "hasAcceptedNutritionDisclaimer")
    ...
    private func openFoodPhotoScanner() {
        let hasAccepted = UserDefaults.standard.bool(forKey: "hasAcceptedNutritionDisclaimer")
        guard hasAccepted else {
            showDisclaimer = true
            return
        }
        showFoodPhotoScanner = true
    }

    // MARK: - AI motivation toast

    private func triggerMotivation() {
        Task {
    ...
    private func generateAIMessage(for entry: FoodEntry) {
        ...
        if name.contains("chicken") || name.contains("salmon") || name.contains("tuna") || name.contains("egg") || name.contains("turkey") || name.contains("shrimp") || name.contains("steak") {
            message = "Great protein choice. This supports your goals perfectly."
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Scan Meal")
                        ...
                        .foregroundStyle(.white)
                    Text("Barcode or AI photo analysis")
                        .font(.system(size: 12))
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                .padding(14).glassCard(cornerRadius: 14).padding(.horizontal, 20)
                ScrollView {
        ...
        .sheet(item: $portionFor) { item in
            PortionSheet(item: item) { portion in
                ...
                if let last = store.foodEntries.last { onFoodLogged?(last) }
                portionFor = nil
                dismiss()
            }
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 10) {
                    ...
                    Button {
                        ...
                        Text("Discard").font(.system(size: 15, weight: .medium)).foregroundStyle(Theme.textSecondary)
                    }
                    .buttonStyle(.plain)
                }
