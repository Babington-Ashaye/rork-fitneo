...
struct FitneoOnboardingView: View {
    ...
    private var personalInfo: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                HStack {
                    Text("Gender").font(.system(size: 13, weight: .semibold)).foregroundStyle(Theme.textSecondary)
                    Spacer()
                    unitToggle($gender, ["male", "female"])
                }
    ...
    private func finish() {
        ...
        store.user.calorieGoal = tdee

        // Save to Supabase with full profile + TDEE
        if let uid = SupabaseService.shared.userId {
            Task {
                ...
                await SupabaseService.shared.saveFullOnboardingProfile(
                    userId: uid,
                    name: name,
                    age: Int(age),
                    gender: gender,
                    ...
                    heightCm: height,
                    goal: goals.first ?? "Stay Active",
                    fitnessLevel: level.title,
                    activityLevel: activityLevel ?? "moderately active",
                    dietaryPreference: dietaryPrefs.first ?? "standard",
    ...
    // MARK: - Mifflin-St Jeor TDEE Calculator
    ...
    private func calculateTDEE() -> Int {
        ...
        case "Very active", "Physical job or sport daily":
            activityMultiplier = 1.725
        case "Moderately active", "On my feet most of the day":
            activityMultiplier = 1.55
        case "Lightly active", "Some walking and daily movement":
        ...
        let tdee = bmr * activityMultiplier * goalMultiplier
        return max(1200, Int(tdee.rounded()))
    }
}
